<template>
  <span class="json-value-wrapper">
    <span
      ref="valueRef"
      :class="['json-value json-number', { editable: isEditable, editing: isEditing }]"
...
const handleSubmit = async () => {
  ...
  try {
    if (props.onChange) {
      ...
      const isValidNumber = newValue.trim() !== '' && !isNaN(Number(newValue))
      
      const changeData = {
        old: { type: 'number', value: props.value },
        new: isValidNumber 
...
const handleContextMenu = (e: MouseEvent) => {
  ...
  if (showConversionMenu) {
    ...
    showConversionMenu({
      ...
      value: props.value,
      availableConversions: getAvailableConversions(props.value, 'number'),
